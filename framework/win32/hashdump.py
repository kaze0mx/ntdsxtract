...
def get_user_hashes(user_key, hbootkey):
    ...
    # Extract hashes from the V value
    if hash_offset + 0x28 < len(V):
        lm_hash_offset = hash_offset + 4
        nt_hash_offset = hash_offset + 8 + 0x10
        enc_lm_hash = V[lm_hash_offset:lm_hash_offset+0x10]
...
def dump_hashes(sysaddr, samaddr):
    ...
    for user in get_user_keys(samaddr):
        ...
        print("{}:{}:{}:{}:::".format(get_user_name(user), int(user.Name,16),
